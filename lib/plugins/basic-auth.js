...
var customFormScriptEmbed = (function() {
  var scriptEmbed = fs.readFileSync(__dirname + '/../../scripts/basic-auth-client.js').toString();
  return '<script>' +
    uglify.minify(scriptEmbed, {fromString: true}).code +
    '</script>';
})();
  ...
  function customLoginForm(req, res, next) {
    ...
    if (_.isFunction(req.ext.loadPageMiddleware)) {
      ...
      req.ext.webPageStream = req.app.settings.storage.readFileStream(loginPath)
        ...
        .on('missing', function() {
          // If the login page is missing, show the browser login
          return browserLoginPrompt(req, res, next);
        });
