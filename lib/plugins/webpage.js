  ...
  return function(req, res, next) {
    ...
    if (req.originalUrl !== '/' && options.canonicalRedirects !== false) {
      ...
      if (req.originalUrl.slice(-1 * options.defaultPage.length) === options.defaultPage) {
        ...
        return res.redirect(301, req.originalUrl.slice(0, -1 * options.defaultPage.length) + req.querystring);
      }

      // If the file has a .html extension, 301 redirect to the extensionless version.
      if (extName === options.defaultFileExtension) {
        res.set('Cache-Control', 'no-cache');
        return res.redirect(301, req.originalUrl.slice(0, -1 * options.defaultFileExtension.length) + req.querystring);
      }

      // If the pageName is 'index' then strip it off.
      if (req.originalUrl.slice(-5) === 'index') {
        return res.redirect(301, req.originalUrl.slice(0, -5) + req.querystring);
      }
    }
    ...
    stream.on('missing', function() {
      ...
      async.detectSeries(alternates, function(alt, cb) {
        ...
        if (_.isObject(foundAlternate)) {
          ...
          res.set('Cache-Control', 'no-cache');
          return res.redirect(foundAlternate.code, foundAlternate.url + req.querystring);
        }
