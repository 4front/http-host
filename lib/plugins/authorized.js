...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    if (!routeRule) {
      ...
      if (_.isObject(req.session.user) && _.isEmpty(req.session.user) === false) {
        req.ext.user = req.session.user;
      }
    ...
    if (_.isObject(req.session) === false) {
      return next(Error.http(506, 'The session plugin must be declared ' +
        'prior to the authorized plugin'));
    }
    ...
    // Store the user on the extended request.
    req.ext.user = req.session.user;
  ...
  function isAllowed(allowed, type, user) {
    ...
    if (_.isArray(allowedList) === false) return true;

    var userList = user[type];

    if (_.isArray(userList) === false) return false;
