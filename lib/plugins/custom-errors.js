...
var debug = require('debug')('4front-http-host:custom-errors');
...
module.exports = function(options) {
  ...
  return function(err, req, res, next) {
    ...
    if (_.isFunction(req.ext.loadPageMiddleware)) {
      req.ext.loadPageMiddleware(req, res, function(streamErr) {
        ...
        decorateEmitter(req.ext.webPageStream, err, req, res, next);
        streamResponse(err, req, res);
      });
    } else {
      // Load the custom error page from deployments
      req.ext.webPageStream = function(callback) {
        var storagePath = urljoin(req.ext.virtualApp.appId,
          req.ext.virtualAppVersion.versionId,
          req.ext.webPagePath);

        var emitter = req.app.settings.storage.readFileStream(storagePath)
          .on('stream', function(stream) {
            callback(null, stream);
          });

        return decorateEmitter(emitter, err, req, res, next);
      };
  ...
  function decorateEmitter(emitter, err, req, res, next) {
    // If the custom error page is missing proceed to the fallback error middleware
    emitter.on('missing', function() {
