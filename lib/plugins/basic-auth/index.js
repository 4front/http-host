...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    if (_.isArray(options.credentials)) {
      validCredentials = _.some(options.credentials, function(creds) {
        return credentials.name === creds.username &&
            credentials.pass === options.password;
      });
    } else {
      validCredentials = (credentials.name === options.username &&
        credentials.pass === options.password);
    }
  ...
  function customLoginForm(req, res, next) {
    ...
    storage.fileExists(loginPath, function(err, exists) {
      ...
      if (exists) {
        req.ext.webPageStream = storage.readFileStream(loginPath, exists === 'fallback')
