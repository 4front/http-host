...
(function() {
  ...
  function login() {
    ...
    var isPdf = /\.pdf$/.test(location.pathname);

    if (isPdf) {
      xhr.setRequestHeader('Accept', 'application/pdf');
      xhr.setRequestHeader('X-Override-Accept-Encoding', 'Base64');
    } else {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 0 || xhr.readyState === 4) {
        if (xhr.status === 200) {
          ...
          if (isPdf) {
            document.write('<embed width="100%" height="100%" type="application/pdf" ' +
              'src="data:application/pdf;base64,' + escape(xhr.responseText) + '"></embed>');
          } else {
