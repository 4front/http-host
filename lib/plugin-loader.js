...
var debug = require('debug')('4front:apphost:plugin-loader');
...
module.exports = function(settings) {
  ...
  var pluginDefaultOptions = {
    "express-session": {
      // If the developer doesn't specify a session store or secret,
      // use the parent 4front app settings. This will generally be the
      // case.
      store: settings.sessionStore,
  ...
  return function(pluginName, options, callback) {
    ...
    if (!pluginModule) {
      return callback(Error.create("Could not load plugin " + pluginName, {
    ...
    // If there are default option values for this plugin, merge them
    // in with the values specified in the package.json manifest
    if (pluginDefaultOptions[pluginName]) {
      debug("merging default options to plugin %s", pluginName);
      options = _.merge(pluginDefaultOptions[pluginName], options);
    }
    ...
    if (_.isFunction(pluginFunction) === false || validateMiddlewareSignature(pluginFunction) === false) {
