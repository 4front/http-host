...
module.exports = function(settings) {
  return function(pluginName, options, callback) {
    ...
    try {
      ...
      return callback(Error.create('Could not require plugin ' + pluginName + ': ' + err.message, {
        plugin: pluginName,
    ...
    if (_.isFunction(pluginModule) === false || pluginModule.length !== 1) {
      ...
      return callback(Error.create(
        'Plugins must export a function with a single options argument.', {
          log: false,
          plugin: pluginName,
  ...
  function requirePlugin(pluginName) {
    if (_.includes(settings.builtInPlugins, pluginName)) {
    ...
    debug('require standard node_modules plugin %s', pluginName);
