...
module.exports = function(settings) {
  ...
  router.use(function(req, res, next) {
    if (_.isFunction(settings.enableContentCache)) {
      req.ext.contentCacheEnabled = settings.enableContentCache(req);
    } else {
      req.ext.contentCacheEnabled = true;
    }

    if (_.isFunction(settings.enableAppCache)) {
      req.ext.appCacheEnabled = settings.enableAppCache(req);
    } else {
      req.ext.appCacheEnabled = true;
    }
  ...
  router.get('/__metadata', function(req, res, next) {
    var settingKeys = ['s3Region', 'dynamoDbRegion', 'dynamoDbRegionFallback',
      'deploymentsBucket', 's3RegionFallback', 'cacheHost', 'contentCacheHost'];

    // Force the __metadata response to not get cached.
    req.ext.contentCacheEnabled = false;

    res.set('Cache-Control', settings.noCacheHttpHeader);
    res.json({
      awsRegion: req.ext.awsRegion,
      settings: _.pick(req.app.settings, settingKeys),
      databaseOptions: _.pick(req.app.settings.database.options, 'tablePrefix', 'region'),
