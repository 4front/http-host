...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    // If the deployedAssetsPath starts with a slash, then register a route
    // to serve the static assets from the Express app. Static asset requests that
    // aren't handled by this middleware will get handled by the static-asset-redirect
    // which comes next. Put this
    if (req.app.settings.deployedAssetsPath[0] === '/') {
      var routePattern = urljoin(req.app.settings.deployedAssetsPath, ':appId', ':versionId', "*");
      router.use(routePattern, function(req, res, next) {
        req.app.deployer.serve(req.ext.virtualApp.appId, req.ext.virtualAppVersion.versionId, req.params[0], res);
      });
    }
