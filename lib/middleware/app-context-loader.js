...
exports = module.exports = function(settings) {
  return function(req, res, next) {
    ...
    router(req, res, function(err) {
      if (req.ext.startAppContextLoad) {
        settings.metrics.timing('app-context-load-time', perfnow() - req.ext.startAppContextLoad);
      }
  ...
  function loadContextFromCache(req, res, next) {
    ...
    settings.cache.get(cacheKey, function(err, json) {
      ...
      // equivalent.
      if (req.ext.virtualApp && req.ext.virtualApp.requireSsl === true && req.secure !== true) {
  ...
  function writeToCache(req, res, next) {
    if (req.ext.appCacheEnabled === false) return next();
  ...
  function loadApplication(req, res, next) {
    ...
    req.ext.startAppContextLoad = perfnow();
  ...
  function loadVersion(req, res, next) {
    ...
    loadVersionFromDatabase(req.ext.virtualApp.appId, versionId, function(err, version) {
      if (err) return next(err);
  ...
  function loadVersionFromDatabase(appId, versionId, callback) {
    async.waterfall([
      function(cb) {
        settings.database.getVersion(appId, versionId, cb);
      },
      function(version, cb) {
        if (version) return cb(null, version);
        if (!settings.databaseFallback) return cb(null, null);

        settings.metrics.increment('dynamo-fallback-read');
        settings.databaseFallback.getVersion(appId, versionId, cb);
      }
    ], callback);
  }
