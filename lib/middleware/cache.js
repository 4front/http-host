...
module.exports = function(settings) {
  ...
  return function(req, res, next) {
    ...
    onHeaders(res, function() {
      ...
      if (!res.getHeader('Cache-Control')) {
        res.setHeader('Cache-Control', settings.noCacheHttpHeader);
      }
  ...
  function bypassCache(req, res) {
    ...
    if (req.query.__nocache === 'true') return true;
  ...
  function generateETag(req) {
    ...
    var entity = JSON.stringify(req.ext.env) + '-' +
      normalizedUrl + '-' +
      accept.encoding(['gzip', 'deflate', 'base64', 'identity']);
    ...
    // Use the versionId as the prefix. Since the content cache on the server
    // uses the etag (minus quotes) as the cache key, this makes it possible to
    // find all the keys by prefix for a specific version.
    return '"' + req.ext.virtualAppVersion.versionId + '__' + hash + '"';
  }
  ...
  function loadHeadersFromCache(req, res, next) {
    ...
    settings.contentCache.hgetall(cacheKey, function(err, headers) {
      ...
      if (!headers) {
        ...
        writeHitMissHeader('miss', req, res);
        return next();
      }
      ...
      if (headers.statusCode === '301' || headers.statusCode === '302') {
        ...
        writeHitMissHeader('hit', req, res);
  ...
  // Only write the hit/miss header in development. Not reliable in
  // production where there is a CDN that will not always return the
  // value from the origin server.
  function writeHitMissHeader(value, req, res) {
    if (process.env.NODE_ENV === 'development') {
      res.set('content-cache', value + ' ' + req.__cacheKey);
    }
  }
