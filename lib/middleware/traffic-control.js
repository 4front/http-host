  ...
  return function(req, res, next) {
    ...
    async.series([
      function(cb) {
        ...
        options.database.getVersionInfo(versionInfo.versionId, function(err, version) {
      ...
      function(cb) {
        ...
        try {
          ...
          debug("error from versionRouter");
          return cb(Error.http(404, err.message, err.data));
        }

        options.database.getVersionInfo(versionId, function(err, versionInfo) {
          if (err) return cb(err);
