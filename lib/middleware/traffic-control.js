  ...
  function getMostRecentVersion(req, callback) {
    ...
    async.series([
      function(cb) {
        ...
        settings.database.mostRecentVersion(req.ext.virtualApp.appId, function(err, version) {
          if (err) return cb(err);
      ...
      function(cb) {
        ...
        settings.database.updateTrafficRules(req.ext.virtualApp.appId, req.ext.virtualEnv,
          [{versionId: mostRecentVersion.versionId, rule: '*'}], cb);
      }
    ], function(err) {
      if (err) return callback(err);
