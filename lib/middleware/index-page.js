...
var debug = require('debug')('4front:index-page');
...
exports = module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    var stream = options.readPageStream(req.ext.virtualApp, 
      req.ext.virtualAppVersion, pageName, next);
    ...
    if (_.isFunction(options.cacheControlFn))
      cacheControl = options.cacheControlFn(req);

    if (_.isEmpty(cacheControl))
      cacheControl = 'no-cache';

    res.set('Cache-Control', cacheControl);
    res.set('Content-Type', 'text/html');
    // Set a custom HTTP header with the version of this app.
    res.set('Virtual-App-Version', req.ext.virtualAppVersion.versionId);
