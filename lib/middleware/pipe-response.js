...
module.exports = function(options) {
  ...
  function conditionalGet(req, res, next) {
    ...
    req.ext.contentCacheEnabled = _.isFunction(req.app.enableContentCache) &&
      req.app.enableContentCache(req) === true;
    ...
    req.app.settings.contentCache.exists(etagHeader, function(err, exists) {
      ...
      if (exists) {
        // TODO: Use a custom CloudWatchMetric to keep track of content cache hit rate.
        debug('webpage exists in cache');
        return req.app.settings.contentCache.readStream(etagHeader).pipe(res);
      }

      debug('webpage does not exist in cache, load from storage');
      pipeFromStorage(req, res, next);
    });
  }
  ...
  function pipeFromStorage(req, res) {
    ...
    if (etagHeader && req.ext.contentCacheEnabled === true) {
      debug('writing page to content cache');
