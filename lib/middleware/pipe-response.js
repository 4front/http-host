...
module.exports = function(options) {
  ...
  function buildHtmlOptions(req, res) {
    ...
    _.extend(htmlOptions, {
      ...
      pathFromRoot: getPathFromRoot(req.path)
  ...
  function getPathFromRoot(reqPath) {
    var pathFromRoot;
    if (_.endsWith(reqPath, '/')) {
      pathFromRoot = path.dirname(reqPath.split('/').slice(0, -1).join('/'));
    } else {
      pathFromRoot = path.dirname(reqPath);
    }
    if (pathFromRoot === '.' || pathFromRoot === '/') pathFromRoot = '';
    return pathFromRoot;
  }
};
