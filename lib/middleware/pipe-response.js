...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    var htmlOptions = buildHtmlOptions(req, versionId, versionName);
    ...
    if (_.isFunction(req.ext.webPageStream)) {
      req.ext.webPageStream(function(err, readStream) {
        if (err) return next(err);
        pipeToResponse(readStream, htmlOptions, res);
      });
    } else {
      pipeToResponse(req.ext.webPageStream, htmlOptions, res);
    }
  };

  function pipeToResponse(stream, htmlOptions, res) {
    if (options.contentType === 'text/html' && options.htmlprep !== false) {
      debug('piping page stream through htmlprep');
      stream.pipe(htmlprep(htmlOptions)).pipe(res);
    } else {
      stream.pipe(res);
    }
  }
  ...
  function buildHtmlOptions(req, versionId, versionName) {
    ...
    if (_.isString(options.clientConfigVar) && req.app.settings.clientConfigVar !== options.clientConfigVar) {
      headBlock += req.app.settings.clientConfigVar + '=';
    }
    headBlock += options.clientConfigVar + '=';
