...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    var doConditionalGet = req.method === 'GET' &&
      (!_.isNumber(res.statusCode) || res.statusCode === 200) &&
      !req.query.__instrument;
  ...
  function conditionalGet(req, res, next) {
    ...
    if (!req.ext.contentCacheEnabled) {
      debug('content cache is not enabled');
      return pipeFromStorage(req, res, next);
    }

    // Trim off the surrounding quotes from the etag
    req.ext.contentCacheKey = etagHeader.slice(1, -1);
    var contentCache = req.app.settings.contentCache;

    async.series([
      function(cb) {
        debug('check if content exists in cache with key %s', req.ext.contentCacheKey);
        contentCache.exists(req.ext.contentCacheKey, function(err, exists) {
          if (err) return cb(err);

          if (exists) {
            res.set('X-Server-Cache', 'HIT');
            debug('webpage read from cache with key %s', req.ext.contentCacheKey);
            req.ext.cacheHit = true;
            return contentCache.readStream(req.ext.contentCacheKey).pipe(res);
          }
          cb();
        });
      },
      function(cb) {
        debug('check if there is a cached redirect');
        contentCache.get(req.ext.contentCacheKey + '-redirect', function(err, redirect) {
          if (err) return cb(err);

          if (_.isString(redirect)) {
            var redirectSplit = redirect.split(' ');
            res.set('Cache-Control', 'no-cache');
            res.set('X-Server-Cache', 'HIT');
            res.redirect(parseInt(redirectSplit[0], 10), redirectSplit[1]);
          }
          cb();
        });
      },
  ...
  function pipeFromStorage(req, res, next) {
    ...
    // Check if the webpage should be written to the content cache. The output
    // includes all the transformations made by htmlprep.
    var writeToCache = req.ext.contentCacheKey &&
      req.ext.contentCacheEnabled === true &&
      req.ext.webPageMissing !== true &&
      req.ext.webPageReadError !== true;

    debugger;
    // Stream the content to the cache on the way through to the response.
    if (writeToCache) {
      debug('writing webpage to cache with key %s', req.ext.contentCacheKey);
      stream = stream.pipe(req.app.settings.contentCache.writeThrough(req.ext.contentCacheKey));
    }
