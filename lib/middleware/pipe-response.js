...
module.exports = function(options) {
  ...
  function conditionalGet(req, res, next) {
    ...
    // TODO: Temporary test to conditionally enable the content cache.
    req.ext.contentCacheEnabled =
      _.isFunction(req.app.settings.enableContentCache) &&
      req.app.settings.enableContentCache(req) === true;
    ...
    // First try reading the page from the cache keyed off the etag
    debug('check if content exists in cache with key %s', etagHeader);
    req.app.settings.contentCache.exists(etagHeader, function(err, exists) {
      if (err) return next(err);

      if (exists) {
        req.app.settings.logger.info('webpage read from cache', {
          type: 'contentCache',
          hit: true,
          url: (req.secure ? 'https' : 'http') + '://' + req.hostname + req.originalUrl,
          key: etagHeader
        });
        req.ext.cacheHit = true;
        return req.app.settings.contentCache.readStream(etagHeader).pipe(res);
      }

      req.app.settings.logger.info('webpage does not exist in cache, load from storage', {
        type: 'contentCache',
        hit: false,
        url: (req.secure ? 'https' : 'http') + '://' + req.hostname + req.originalUrl,
        key: etagHeader
  ...
  function pipeFromStorage(req, res) {
    ...
    // If there's an etag header, write it to the cache on the way out. The output
    // includes all the transformations made by htmlprep.
    var etagHeader = res.getHeader('etag');
    if (etagHeader && req.ext.contentCacheEnabled === true) {
      req.app.settings.logger.info('writing webpage to cache', {
        type: 'contentCache',
        url: (req.secure ? 'https' : 'http') + '://' + req.hostname + req.originalUrl,
        key: etagHeader
      });
      stream = stream.pipe(req.app.settings.contentCache.writeThrough(etagHeader));
    }
