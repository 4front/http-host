    ...
    _.extend(req.ext, {
      ...
      assetStorage: {
        createReadStream: function(appId, versionId, pageName) {
          ...
          var exists = options.cache.exists(cacheKey);

          debug('streaming %s from cache, exists=%s', cacheKey, exists);
          return options.cache.readStream(cacheKey)
            .on('readable', function() {
              if (exists !== true) {
                debug('cache key %s does not exist', cacheKey);
                this.emit('missing', Error.create('Page ' + pageName + ' not found in simulator cache.', {
