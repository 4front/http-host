  ...
  return function(req, res, next) {
    ...
    req.app.settings.cache.get(req.ext.user.userId + '/' + req.ext.virtualApp.appId + '/_manifest', function(err, manifestJson) {
      ...
      _.merge(req.ext, {
        ...
        htmlOptions: {
          liveReload: devOptions.liveReload === '1', // use custom embedded lr server, see below.
      ...
      //   debug('injecting livereload snippet');
      //   // Inject custom livereload snippet that uses a relative path to the
      //   // embedded tiny-lr server.
      //   var lrOptions = {
      //     host: 'localhost',
      ...
      //   };
      //
      //   // Inject a global LiveReloadOptions variable to have tighter control
      //   // over how tinylr gets options on the client side.
      //   // req.ext.htmlOptions.inject.body = "<script>window.LiveReloadOptions=" +
      //   //   JSON.stringify(lrOptions) + ";</script>" +
      //   //   "<script src='//localhost:" + devOptions.port + "/livereload/livereload.js'></script>";
      // }
