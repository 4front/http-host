...
exports = module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    if (_.isEmpty(devOptions.user))
      return next(Error.http(400, "No user parameter found in the found in the " + options.cookieName + " cookie.", {
        bypassCustomErrorPage: true,
        help: "Try restarting the dev sandbox"
      }));
    ...
    // Augment the extended request with a custom implementation of assetStorage
    // so the html-page middleware will use
    _.extend(req.ext, {
      // Specify a custom createReadStream function for the html-page middleware
      // to use rather than the default assetStorage.createReadStream
      createReadStream: function(pageName, callback) {
        ...
        req.app.settings.cache.exists(cacheKey, function(err, exists) {
          ...
          if (!exists) {
            ...
            return callback(Error.http(404, 'Page ' + pageName + ' not found in sandbox cache.', {
              bypassCustomErrorPage: true,
              help: 'Try restarting the development sandbox.'
            }));
          }

          callback(null, req.app.settings.cache.readStream(cacheKey));
        });
      },
      virtualAppVersion: {
        versionId: 'sandbox',
