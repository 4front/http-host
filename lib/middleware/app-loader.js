...
var CACHE_HIT_METRIC = 'app-cache-hit';
var CACHE_MISS_METRIC = 'app-cache-miss';
...
exports = module.exports = function(settings) {
  ...
  function loadContextFromCache(req, res, next) {
    ...
    settings.cache.get(cacheKey, function(err, json) {
      if (err || !json) {
        ...
        settings.metrics.increment(CACHE_MISS_METRIC);
        return next();
      }
      ...
      settings.metrics.increment(CACHE_HIT_METRIC);

      if (appContext.redirect) {
        return res.redirect(appContext.redirect.statusCode, appContext.redirect.location);
      }
  ...
  function writeToCache(req, res, next) {
    if (req.ext.appCacheHit === true) return next();

    // Write certain properties to the cache.
    settings.cache.setex(req.ext.virtualHost, CACHE_TTL,
  ...
  function finalizeContext(req, res, next) {
    ...
    debug('current app context appId=%s, versionId=%s',
      req.ext.virtualApp.appId, version.versionId);
    next();
  }
