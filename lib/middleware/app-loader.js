...
exports = module.exports = function(settings) {
  return function(req, res, next) {
    ...
    if (req.ext.virtualHost.indexOf(settings.virtualHost) === -1) {
      debug('find app by custom domain %s', virtualHost);
      settings.virtualAppRegistry.getByDomain(req.ext.virtualHost, appRegistryCallback);
    } else {
      ...
      settings.virtualAppRegistry.getByName(appName, appRegistryCallback);
    }
    ...
    function appRegistryCallback(err, virtualApp) {
      ...
      if (virtualApp.domain && virtualApp.domain.action === 'redirect') {
        ...
        return res.redirect(302, virtualApp.url);
      }

      // If this app requires SSL and this request is not secure, then redirect to the https
      // equivalent. If the app is being requested with a custom domain, then ensure the domain
      // has a SSL certificate.
      if (virtualApp.requireSsl === true && (!virtualApp.domain || _.isString(virtualApp.domain.certificate))) {
  ...
  function determineVirtualEnv(req) {
    var match = req.hostname.match(/--([a-z0-9]+)\./i);
