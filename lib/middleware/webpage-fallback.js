  ...
  return function(req, res, next) {
    ...
    async.detectSeries(alternates, function(alt, cb) {
      ...
      if (_.isObject(foundAlternate)) {
        ...
        // Store the content redirect in the content cache
        if (req.ext.contentCacheEnabled) {
          req.app.settings.contentCache.set(req.ext.contentCacheKey + '-redirect',
            foundAlternate.code + ' ' + foundAlternate.url);
        }
