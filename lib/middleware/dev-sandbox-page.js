...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    var cacheKey = urljoin(req.ext.user.userId, req.ext.virtualApp.appId, req.ext.webPagePath);
    ...
    if (req.cookies[options.cookieName]) {
      ...
      req.app.settings.cache.exists(cacheKey, function(err, exists) {
        ...
        if (!exists)
          return next(Error.http(404, 'Page ' + req.ext.webPagePath + ' not found.', {code: "pageNotFound"}));
