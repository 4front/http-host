...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    var cacheKey = urljoin(req.ext.user.userId, req.ext.virtualApp.appId, req.ext.htmlPagePath);
    ...
    if (req.cookies[options.cookieName]) {
      ...
      req.app.settings.cache.exists(cacheKey, function(err, exists) {
        ...
        if (!exists)
          return next(Error.http(404, 'Page ' + req.ext.htmlPagePath + ' not found.', {code: "pageNotFound"}));
      ...
      var redirectUrl = {
        ...
        hostname: 'localhost',
        port: options.port,
        pathname: urljoin('sandbox', req.ext.htmlPagePath)
      };
      ...
      async.parallel({
        ...
        lastModified: function(cb) {
          req.app.settings.cache.get(cacheKey + '/mtime', cb);
        }
        ...
        // the file if the lastModified time is different.
        if (results.exists && _.isEmpty(results.lastModified) === false)
          redirectUrl.query.mtime = results.lastModified;
