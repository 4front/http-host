...
var debug = require('debug')('4front:apphost:page-stream');
...
exports = module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    var stream = options.readPageStream(req.ext.virtualApp, req.ext.virtualAppVersion, req.ext.htmlPageName);
    stream.on('missing', function(err) {
    ...
    _.extend(options.htmlprep, {
      ...
      inject: {
        ...
        head: '<script>' + options.clientConfigVar + '=' + JSON.stringify(req.ext.clientConfig) + ';</script>' 
      }
    });
