...
module.exports = function(options) {
  options = _.defaults({}, options, {
    ...
    replaceBaseUrlFileExtensions: ['.xml', '.json', '.txt'],
  ...
  return function(req, res, next) {
    ...
    customHeaders(req, res, function(err) {
      ...
      var acceptsGzip = req.acceptsEncodings(['gzip']) === 'gzip';
  ...
  function pipeAssetToResponse(req, res, next) {
    ...
    storage.fileExists(req.ext.staticFilePath, function(err, exists) {
      if (err) return next(err);
      ...
      var fileStream = storage.readFileStream(req.ext.staticFilePath, exists === 'fallback')
        .on('metadata', function(metadata) {
          ...
          if (metadata.contentLength && !base64Encode) {
        ...
        .on('readError', function(readError) {
          debug('error from storage.createReadStream', readError.message);
      ...
      if (_.includes(options.replaceBaseUrlFileExtensions, req.ext.extName) && isGzipped !== true) {
        ...
        fileStream = fileStream.pipe(replaceStream(baseUrlRegex, function(match) {
          if (_.endsWith(match, '/')) {
            return baseUrl + '/';
          }
          return baseUrl;
      ...
      if (base64Encode) {
        ...
        fileStream = fileStream.pipe(base64.encode());
      }
