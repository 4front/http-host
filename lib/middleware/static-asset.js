...
module.exports = function(settings) {
  ...
  function pipeAssetToResponse(filePath, req, res, next) {
    ...
    var readStream = settings.storage.readFileStream(filePath)
    ...
    // HACK: need to delay to give time for gunzipStream to potentially
    // be set to true in the 'metadata' event. The through stream is
    // necessary to start the flow of data to force the 'metadata' event to
    // be triggered.
    readStream = readStream.pipe(through());

    process.nextTick(function() {
      if (gunzipStream === true) {
        debug('gunzip the response');
