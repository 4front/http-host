...
module.exports = function(options) {
  ...
  function pipeAssetToResponse(req, res, next) {
    ...
    // Pipe it through the replace stream twice in order to fix any double slash
    // issues.
    if (_.includes(options.replaceBaseUrlFileExtensions, req.ext.extName) && isGzipped !== true) {
      ...
      fileStream
        .pipe(replaceStream(options.baseUrlPlaceholder, baseUrl))
        .pipe(replaceStream(baseUrl + '//', baseUrl + '/'))
        .pipe(res);
    } else {
      fileStream.pipe(res);
    }
