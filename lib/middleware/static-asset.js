...
module.exports = function(settings) {
  ...
  function pipeAssetToResponse(filePath, req, res, next) {
    ...
    var readStream = settings.storage.readFileStream(filePath)
      .on('metadata', function(metadata) {
        ...
        // along in a header.
        gzipEncoded = metadata.contentEncoding === 'gzip';

        if (gzipEncoded === true) {
          var accept = accepts(req);
          if (accept.encoding(['gzip']) === false) {
            gunzipResponse(res);
          } else {
  ...
  // Code taken from expressjs/compression and modified slightly for
  // gunzip rather than gzipping.
  function gunzipResponse(res) {
    var ended = false;
    var write = res.write;
    var on = res.on;
    var end = res.end;
    var responseStream = zlib.createGunzip();
