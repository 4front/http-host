...
module.exports = function(settings) {
  ...
  return function(req, res, next) {
    ...
    if (path.extname(req.path).length === 0 || path.extname(req.path) === '.html') {
      return next();
    }
    ...
    req.ext.contentType = mime.lookup(req.ext.staticFilePath);
    req.ext.extname = path.extname(req.path);
    res.setHeader('Content-Type', req.ext.contentType);
    ...
    // support gzip.
    if (!acceptsGzip || path.extname(req.path) === '.xml') {
      ...
      req.app.settings.storage.getMetadata(req.ext.staticFilePath, function(err, metadata) {
        ...
        if (metadata.contentEncoding === 'gzip') {
          return settings.storage.readFileStream(req.ext.staticFilePath)
  ...
  function pipeAssetToResponse(req, res, next) {
    ...
    var fileStream = settings.storage.readFileStream(req.ext.staticFilePath)
    ...
    // If this is an xml file, read through it line by line and
    // replace any instances of __baseurl__ with the actual site url.
    if (path.extname(req.path) === '.xml') {
      var baseUrl = (req.secure ? 'https' : 'http') + '://' + req.hostname;
      fileStream = byline.createStream(fileStream);
      fileStream.on('data', function(line) {
        res.write(line.toString().replace(settings.baseUrlPlaceholder, baseUrl) + '\n');
      });
