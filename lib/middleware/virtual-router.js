...
module.exports = function(options) {
  helper.ensureRequiredOptions(options, 'pluginsDir');

  // if (!options.pluginLoaderFn)
  //   throw new Error('Missing required option pluginLoader');
  ...
  function middlewareFn(routeInfo) {
    return function(req, res, next) {
      ...
      if (routeInfo.module.slice(0, 8) === 'plugin::') {
        moduleRequirePath = path.join(options.pluginsDir, routeInfo.module.slice(8));
      }
      else {
