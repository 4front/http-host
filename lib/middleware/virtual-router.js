...
var BUILT_IN_PLUGINS = ['basic-auth', 'authorized',
  ...
  'redirect', 'session', 'webpage', 'http-headers', 'prerender'];

// Names of the top-level manifest properties that should
// be passed along to every plugin.
var TOP_LEVEL_PLUGIN_OPTS = ['clientConfigVar', 'omitClientConfigVar'];
...
module.exports = function(settings) {
  ...
  var cacheMiddleware = require('../cache')(settings);
  var staticAssetMiddleware = require('./static-asset')(settings);
  var ignoreRequest = require('./ignore-request')(settings);
  var defaultResponse = require('./default-response')(settings);
  ...
  return function(req, res, next) {
    ...
    if (settings.autoIncludeWebpagePlugin && _.some(routerConfig, {module: 'webpage'}) === false) {
      routerConfig.push({
        method: 'GET',
        module: 'webpage'
      });
  ...
  function routeValidForEnv(req, routeInfo) {
    ...
    // appears in the list.
    return _.some(routeInfo.environments, function(env) {
    ...
    for (var i = 0; i < routerConfig.length; i++) {
      ...
      if (_.isArray(routeInfo.path)) {
        for (var pathIndex = 0; pathIndex < routeInfo.path.length; pathIndex++) {
          if (routeInfo.method) {
            debug('router.' + routeInfo.method + '("' + routeInfo.path[pathIndex] +
              '", ' + routeInfo.name + ')');
  ...
  function categorizeRoutes(routerConfig) {
    ...
    for (var i = 0; i < routerConfig.length; i++) {
      ...
      if (routeInfo.path && _.isString(routeInfo.method) === true) {
        ...
        if (_.includes(VALID_METHODS, routeInfo.method) === false) {
          var msg = 'Invalid method ' + routeInfo.method + ' for virtual route ' + routeInfo.path;
          throw Error.http(500, msg, {
  ...
  function lazyMiddlewareWrapper(routeInfo) {
    ...
    // Return an instrumented instance of the plugin middleware function
    return instrument.middleware(function(req, res, next) {
