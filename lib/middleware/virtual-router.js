...
module.exports = function(options) {
  ...
  function middlewareFn(routeInfo) {
    return function(req, res, next) {
      ...
      pluginLoader(routeInfo.module, routeInfo.options, function(err, pluginFunction) {
        if (err) {
          debug("could not load plugin %s: %s", routeInfo.module, err.stack);

          return next(Error.http(501, "Could not load plugin", {
            code: err.code,
