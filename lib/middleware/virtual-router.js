...
module.exports = function(settings) {
  ...
  return function(req, res, next) {
    ...
    // Insert the static-asset middleware immediately before the webpage
    ...
    // Put the ignore-request and catchAll404 between the standard routes
    // and error routes. This provides the custom plugins a chance to handle
    // what are ignored requests that return a 204 response. For example
    // redirecting .php requests to an extension-less equivalent. If the request
    // makes it past the ignore-request middleware, then pass down a 404
    // error to any custom error middleware.
    router.use(require('./ignore-request')(settings));
