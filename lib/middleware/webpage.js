...
module.exports = function(options) {
  ...
  return function(req, res, next) {
    ...
    if (options.pushState === true || actualUrl.pathname === '/')
      req.ext.pagePath = options.defaultPage;
    else
      req.ext.pagePath = actualUrl.pathname.substr(1); // chop off leading slash
    ...
    if (ensureAuthenticated && req.ext.isAuthenticated !== true) {
      ...
      if (_.isEmpty(options.noAuthPage) === false) {
        if (actualUrl.pathname === '/')
          req.ext.pagePath = options.noAuthPage;
        else {
    ...
    if (path.extname(req.ext.pagePath).length === 0)
      req.ext.pagePath += options.defaultFileExtension;
    ...
    if (_.isFunction(req.ext.loadPageMiddleware)) {
      ...
      req.ext.pageStream = req.app.settings.deployments.readFileStream(
    ...
    function streamResponse() {
      ...
      req.ext.pageStream.on('missing', function() {
      ...
      res.set('Virtual-App-Page', req.ext.pagePath);
      ...
      if (options.contentType === 'text/html' && options.htmlprep !== false) {
        ...
        req.ext.pageStream = req.ext.pageStream.pipe(htmlprep(htmlOptions));
      }
  ...
  function buildHtmlOptions(req, versionId, versionName) {
    ...
    if (!versionAssetPath) {
      versionAssetPath = urljoin(req.app.settings.staticAssetPath, req.ext.virtualApp.appId, versionId);
    }

    _.extend(req.ext.clientConfig, {
      buildType:  req.ext.buildType || 'release',
      pagePath: req.ext.pagePath,
      user: req.user,
    ...
    }, _.pick(req.ext.htmlOptions || {}, 'liveReload', 'staticAssetPath'));
